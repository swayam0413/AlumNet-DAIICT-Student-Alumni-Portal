...
export default function Jobs() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      if (data) {
        setJobs(data);
        // Pre-fetch alumni for all unique companies
        const uniqueCompanies = [...new Set(data.map(j => j.company).filter(Boolean))];
        const alumniMap: Record<string, UserProfile[]> = {};
        await Promise.all(
          uniqueCompanies.map(async (company) => {
            try {
              const alumni = await dataService.getAlumniByCompany(company);
              alumniMap[company] = alumni;
            } catch {
              alumniMap[company] = [];
            }
          })
        );
        setCompanyAlumni(prev => ({ ...prev, ...alumniMap }));
      }
    } catch (error) {
  ...
  const handlePostJob = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // --- Notification: Notify all students about new job ---
      try {
        await dataService.notifyUsersByRole('student', {
          title: '💼 New Job Opportunity!',
          message: `${profile.name} posted a new role: ${newJob.title} at ${newJob.company}`,
          type: 'JOB',
          actionUrl: '/jobs',
          icon: '💼',
        });
        // Also notify alumni
        await dataService.notifyUsersByRole('alumni', {
          title: '💼 New Job Posted',
          message: `${profile.name} posted: ${newJob.title} at ${newJob.company}`,
          type: 'JOB',
          actionUrl: '/jobs',
          icon: '💼',
        });
        ...
        dataService.sendEmailNotification(
          [], // Backend will need user emails — for now this triggers the endpoint
          `New Job: ${newJob.title} at ${newJob.company}`,
          `<h2>New Job Opportunity on AlumConnect!</h2>
           <p><strong>${newJob.title}</strong> at <strong>${newJob.company}</strong></p>
           <p>${newJob.description}</p>
           <p>Posted by: ${profile.name}</p>
           <a href="http://localhost:5173/jobs">View Jobs →</a>`
        ).catch(() => { });
      } catch (notifErr) {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div className="space-y-4">
          <h2 className="text-[3.5rem] font-extrabold tracking-tight text-on-surface leading-none mb-4 font-headline">
            Job opportunities <br /><span className="text-primary font-headline"></span>
          </h2>
              ...
              <div className="mt-auto pt-4 border-t border-outline-variant/10 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isStudent && (companyAlumni[job.company]?.length > 0) && (
                      <button
